...
public:
  ...
  Matrix4x4 static rotation(double angle, char axis);
  Matrix4x4 static translation(const Vector3D& displacement);
  Matrix4x4 static scaling(const Vector3D& scale);
  Matrix4x4 static projection(double fov, double aspect, double near, double far);
